...
{
	struct ether_addr *addr = &ports_eth_addr[port_id];
...
worker_thread(void *args_ptr)
{
	...
	while (!quit_signal) {
		...
		for (i=0; i<burst_size; i++) {
			...
			dst_portid = burst_buffer[i]->port;
			eth_hdr = rte_pktmbuf_mtod(burst_buffer[i], struct ether_hdr *);
			ether_addr_copy(&ports_eth_addr[dst_portid], &eth_hdr->s_addr); 
		}
